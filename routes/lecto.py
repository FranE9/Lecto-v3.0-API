...
async def upload_file(background_tasks: BackgroundTasks, archivo_pdf: UploadFile = File(...), inicio: int= Form(...), final: int= Form(...), user_id:str=Form(...)):
    Validation_Result()
    ...
    try:
        ...
        background_tasks.add_task(process_pdf_request, new_ticket, Source, inicio, final, user_found["email"])
...
async def process_pdf_request(my_ticket, Source, inicio, final, user_email):
    ...
    email_task = asyncio.create_task(send_email(user_email, f"[LECTO] Creacion de ticket {new_ticket.id} finalizada", f"Ticket {new_ticket.id} ya esta listo para ser consultado."))
